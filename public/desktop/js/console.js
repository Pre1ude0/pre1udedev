  ...
  if (splitCommand[0] === "clear") {
    ...
    if (splitCommand.length === 2) {
      if (currentDirectory.children[splitCommand[1]]) {
        if (currentDirectory.children[splitCommand[1]].type === "file") {
          let content =
            currentDirectory.children[splitCommand[1]].content.split("\n");
      ...
      if (currentDirectory.children[splitCommand[1]]) {
        if (currentDirectory.children[splitCommand[1]].type === "file") {
          let content =
            currentDirectory.children[splitCommand[1]].content.split("\n");
          for (
            let i = Math.max(0, content.length - 10);
            i < content.length;
            i++
          ) {
            outputElement.innerHTML += `${content[i]}<br>`;
...
function initializeConsole() {
  ...
  addEventListener("keydown", (e) => {
    if (
      consoleWindow.classList.contains("active") &&
      !keyBlacklist.includes(e.key)
    ) {
      if (e.key === "Enter") {
        if (currentCommand === "") {
